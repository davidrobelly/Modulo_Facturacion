...
import Tablas.Tabla_Producto_Consulta_MarcaModeloCod;
import Tablas.Tabla_Producto_Consulta_Serie;

@SuppressWarnings("serial")
public class Pantalla_Detalle_Producto extends JFrame implements ActionListener, MouseListener{
	...
	public JLabel lbl_categoria, lbl_nombre, lbl_marca, lbl_modelo, lbl_valor,
				  lbl_minimo, lbl_maximo, lbl_serie, lbl_color, lbl_detalle, lbl_codProducto;
	...
	public JButton btn_agregar, btn_modificar, btn_limpiar, btn_salir, btn_buscarMarca, btn_buscarModelo;
	public Tabla_Producto_Consulta_MarcaModeloCod jtproducto;
	public Tabla_Producto_Consulta_Serie jtSerie;
	public JTable tabla;
	...
	public Pantalla_Detalle_Producto (){
		
		super("Ingreso Detalle del Producto");
		setSize(800,500);
		setLocation(400,25);
		...
		lbl_codProducto = new JLabel("Codigo :");
		pnl_datGenPro.add(lbl_codProducto);
		gridSuperior1.setConstraints(lbl_codProducto, gridConSuperior1);
		
		gridConSuperior1.gridx = 1;
		gridConSuperior1.gridy = 7;
		txt_codProducto = new JTextField(20);
		txt_codProducto.setEditable(false);
		pnl_datGenPro.add(txt_codProducto);
		gridSuperior1.setConstraints(txt_codProducto, gridConSuperior1);
		...
		deshabilitarPanelDetalle();
		
		//PANEL TABLA
		pnl_tabla = new JPanel();
		
		jtproducto = new Tabla_Producto_Consulta_MarcaModeloCod();
		...
		gridConInferior.gridy = 0;
		btn_modificar = new JButton(new ImageIcon("src/Imagenes/actualizar.jpeg"));
		btn_modificar.setBorder(null);
		btn_modificar.addActionListener(this);
		pnl_botones.add(btn_modificar);
		gridInferior.setConstraints(btn_modificar, gridConInferior);
		...
		gridConInferior.gridy = 0;
		btn_salir = new JButton(new ImageIcon("src/Imagenes/salir.jpeg"));
	...
	public void tamañoTabla () {
		TableColumnModel tamaño = tabla.getColumnModel();
		for (int i = 0; i < tamaño.getColumnCount(); i++) {
			tamaño.getColumn(i).setPreferredWidth(500);
		}
	}
	...
	public void habilitarPanelDetalle (){
		pnl_datEspePro.setEnabled(true);
		lbl_serie.setEnabled(true);
		txt_serie.setEnabled(true);
		lbl_color.setEnabled(true);
		txt_color.setEnabled(true);
		lbl_detalle.setEnabled(true);
		txta_detalle.setEnabled(true);
	}
	
	public void deshabilitarPanelDetalle (){
		pnl_datEspePro.setEnabled(false);
		lbl_serie.setEnabled(false);
		txt_serie.setEnabled(false);
		lbl_color.setEnabled(false);
		txt_color.setEnabled(false);
		lbl_detalle.setEnabled(false);
		txta_detalle.setEnabled(false);
	}
	...
	public void actionPerformed(ActionEvent evento) {
		...
		if(evento.getSource() == btn_agregar){	
			...
			Producto producto = new Producto(codProducto, serie, color, detalle);
			
			producto.ingresarDetalleProducto(producto);
		...
		if(evento.getSource() == btn_buscarMarca){
			...
			if ("".equals(marcaB)){
				...
				ArrayList<Producto> listaProductoMarcaCod = producto.listarProductosxMarcaCodigo(marcaB);
				Tabla_Producto_Consulta_MarcaModeloCod tablaProMarcaCod = new Tabla_Producto_Consulta_MarcaModeloCod(listaProductoMarcaCod);
				tabla.removeAll();
				tabla.setModel(tablaProMarcaCod);
				tabla.updateUI();
				habilitarPanelDetalle();
			}
		
		
		}
		
		if(evento.getSource() == btn_buscarModelo){
			
			String modeloB;
			modeloB = txt_modelo.getText();
			
			if ("".equals(modeloB)){
				JOptionPane.showMessageDialog(null, "Llenar el campo de busqueda: modelo");
				txt_modelo.setText("");
			} else{
				Producto producto = new Producto();
				ArrayList<Producto> listaProductoModelo = producto.listarProductosxModeloCodigo(modeloB);
				Tabla_Producto_Consulta_MarcaModeloCod tablaProModelo = new Tabla_Producto_Consulta_MarcaModeloCod(listaProductoModelo);
				tabla.removeAll();
				tabla.setModel(tablaProModelo);
				tabla.updateUI();
			}
		}
